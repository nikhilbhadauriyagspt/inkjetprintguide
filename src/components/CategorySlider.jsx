  ...
  return (
    <section className={`px-6 md:px-10 lg:px-12 py-24 ${bgColor} font-urbanist overflow-hidden`}>
      <div className="flex items-end justify-between mb-12 border-b border-gray-100 pb-8">
        <div>
          <span className="text-[10px] font-bold tracking-[0.4em] capitalize text-[#F54900] mb-2 block ml-1">{subtitle}</span>
      ...
      >
              ...
              >
                <Link to={`/product/${p.slug}`} className="relative aspect-square bg-white rounded-2xl overflow-hidden mb-3 border border-gray-100 group-hover:border-blue-500/20 transition-all duration-300 flex items-center justify-center p-6">
                  ...
                  <div className="absolute bottom-2 left-2 right-2 translate-y-[120%] group-hover:translate-y-0 transition-transform duration-300">
                    <button
                      ...
                      className={`w-full h-8 rounded-lg flex items-center justify-center gap-2 text-[9px] font-bold capitalize tracking-widest transition-all shadow-lg ${addedItems[p.id] ? 'bg-emerald-500 text-white' : 'bg-black text-white hover:bg-[#F54900]'}`}
                    >
                ...
                <div className="px-1">
                  <Link to={`/product/${p.slug}`}>
                    <h3 className="text-[11px] font-bold text-slate-800 group-hover:text-[#F54900] transition-colors line-clamp-1 mb-0.5">{p.name}</h3>
                  </Link>
        ...
        <div className="absolute -top-20 right-0 flex gap-2">
          <CarouselPrevious className="static translate-y-0 h-10 w-10 border-gray-100 hover:bg-[#F54900] hover:text-white transition-all" />
          <CarouselNext className="static translate-y-0 h-10 w-10 border-gray-100 hover:bg-[#F54900] hover:text-white transition-all" />
        </div>
      </Carousel>
    </section>
  );
