      ...
      <div className="flex items-center justify-around h-16 px-4">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`relative ${isActive ? 'text-[#1447E6]' : 'text-slate-400'}`}>
                <Icon size={22} strokeWidth={isActive ? 2.5 : 2} />
                {item.name === 'Store' && cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-2 h-4 w-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center ring-2 ring-white">
                    {cartCount}
                  </span>
                )}
              </div>
              <span className={`text-[10px] font-bold capitalize tracking-tighter ${isActive ? 'text-slate-900' : 'text-slate-400'}`}>
                {item.name}
              </span>
            </Link>
          );
