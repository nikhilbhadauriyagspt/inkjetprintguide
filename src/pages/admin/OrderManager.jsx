...
export default function OrderManager() {
  ...
  const statusColors = {
    ...
    processing: 'bg-blue-50 text-[#991B1B] border-blue-100',
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-[#991B1B]">${order.total_amount}</span>
            ...
            >
              <div className="p-8 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-[#991B1B] capitalize tracking-widest mt-1">#PFX-{selectedOrder.id}</p>
                </div>
                    ...
                    {Object.keys(statusColors).map((s) => (
                      <button
                        ...
                        className={`px-4 py-3 rounded-xl text-[9px] font-bold capitalize tracking-widest border transition-all ${selectedOrder.status === s
                          ? 'bg-[#991B1B] border-blue-600 text-white'
                  ...
                  <div className="border border-gray-100 rounded-2xl overflow-hidden divide-y divide-gray-50">
                    {selectedOrder.items?.map((item, idx) => (
                      <div key={idx} className="p-4 flex items-center justify-between bg-white">
                        ...
                        <p className="text-xs font-bold text-[#991B1B]">${item.price}</p>
                      </div>
                    ))}
                  </div>
                </div>
                                ...
                                return (
                                  <>
                                    <div className="flex justify-between">
                                      ...
                                      <span className="text-[9px] font-mono font-bold text-[#991B1B]">{details.id || 'N/A'}</span>
                                    </div>
                                    <div className="flex justify-between">
