        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="hidden lg:block lg:col-span-3 sticky top-8">
                <div className="bg-white p-6 border border-slate-100 rounded-xl shadow-sm">
                    ...
                    <ul className="space-y-4">
                        {faqData.map((cat, idx) => (
                            <li key={idx}>
                                <a href={`#cat-${idx}`} className="text-[14px] text-slate-500 hover:text-blue-600 transition-all flex items-center gap-2 group">
            ...
            <div className="lg:col-span-9 space-y-10">
                    ...
                    return (
                        <div key={catIdx} id={`cat-${catIdx}`} className="bg-white border border-slate-100 rounded-xl shadow-sm overflow-hidden scroll-mt-8">
                            <div className="bg-slate-50 px-8 py-4 border-b border-slate-100">
                                <h2 className="text-[13px] font-bold text-blue-600 uppercase tracking-[0.2em]">{cat.category}</h2>
                            </div>
                                ...
                                {filteredQuestions.map((faq, qIdx) => {
                                    ...
                                    return (
                                        <div key={qIdx} className="group">
                                            ...
                                            >
                                                <span className={`text-[15px] font-bold ${isOpen ? 'text-blue-600' : 'text-slate-900 group-hover:text-blue-600'} transition-colors`}>{faq.q}</span>
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${isOpen ? 'bg-blue-600 text-white rotate-180' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600'}`}>
                ...
                <div className="bg-slate-50 p-10 md:p-12 border border-slate-100 rounded-xl text-center space-y-6">
                    <div className="w-16 h-16 bg-white shadow-sm border border-slate-100 rounded-2xl flex items-center justify-center mx-auto text-blue-600">
                    ...
                    <Link to="/contact" className="inline-flex items-center gap-3 bg-blue-600 hover:bg-blue-700 text-white px-10 py-4 text-[13px] font-bold uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-blue-600/20 active:scale-95">
